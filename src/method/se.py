...
{{
  ...
  "final_answer": "Your final answer"
    ...
    def se_score(self, class_mat, entail_id):
        C = class_mat
        m = C.shape[0]
        entail = (C == entail_id)
    ...
    def calculate_scores(self, path):
        ...
        if self.dataset_name == "truthfulqa_mc":
            ...
            id_map = {
                ...
                "contra_id": find_label_id(["contra", "contradict"]),
            }

        if "class_mat_sc" not in dataset[0]:
            for item in tqdm(dataset):
                sample_texts = []

                for s in item["sample_results"][:self.sample_num]:
                    ...
                    ans = ans.strip()
                    if ans:
                        sample_texts.append(ans)

                if self.dataset_name == "truthfulqa_mc":
                    n = len(sample_texts)
                    class_mat = [[0 for _ in range(n)] for _ in range(n)]

                    for i in range(n):
                        for j in range(n):
                            if sample_texts[i] == sample_texts[j]:
                                class_mat[i][j] = id_map["entail_id"]
                            else:
                                class_mat[i][j] = id_map["contra_id"]
