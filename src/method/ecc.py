...
class Ecc:
    ...
    def __init__(
        ...
        self.save_path = f"{config.OUTPUT_DIR}/ecc/{dataset_name}/{model_name}.json"
...
{{
  ...
  "final_answer": "Your final answer"
    ...
    def compute_eccentricity_uncertainty(self, W: np.ndarray) -> float:
    ...
    def calculate_scores(self, path):
        ...
        if self.similarity_score == "NLI_score" and "semantic_matrix_entail" not in dataset[0].keys():
            for item in tqdm(dataset):
                ...
                sample_texts = []
                for idx, s in enumerate(item["sample_results"][:5]):
                ...
                E, C, N, E_logits, C_logits, N_logits, class_mat, id_map = compute_semantic_matrix(
                    question=question,
        ...
        for item in dataset:
            if self.similarity_score == "NLI_score":
                ...
                if E.ndim != 2 or C.ndim != 2 or E.shape[0] <= valid_num or E.shape[0] != E.shape[1] or C.shape != E.shape:
                    ecc_scores.append(None)
                    continue

                if self.affinity == "entail":
                    W = (E + E.T) / 2.0
                    u = float(self.compute_eccentricity_uncertainty(W))
                else:
                    W = 1.0 - C
                    W = (W + W.T) / 2.0
                    u = float(self.compute_eccentricity_uncertainty(W))
                ...
                texts = [(s.get("text") or "").strip() for s in item["sample_results"][:5]]
    ...
    def calculate(self):
        sample_test_path, sample_dev_path = build_sample_path(self.save_path)
        ecc_scores_dev = self.calculate_scores(sample_dev_path)
        ecc_scores_test = self.calculate_scores(sample_test_path)
        ...
        valid_scores_dev = [
            float(x) for x in ecc_scores_dev
        ...
        for item, ecc_score in zip(dataset, ecc_scores_test):
            if ecc_score is not None and item["greedy_results"]["answer"] is not None:
                ecc_score_norm = minmax(ecc_score)           
                conf = 1.0 - ecc_score_norm 
                output.append({"id": item["id"], "question": item["question"],
                    ...
                    "ecc_score": ecc_score, "pred_score": conf})
            else:
                output.append({"id": item["id"], "question": item["question"],
                    "description": item["description"], "ground_truth": item["ground_truth"],
                    "pred_answer": item["greedy_results"]["answer"],
                    "pred_score": None,
                    "ecc_score": None, "pred_score": None})
