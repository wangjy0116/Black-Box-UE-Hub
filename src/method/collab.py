...
def eval_ans(dataset_name, item, ans, reference, method: str = "gpt_cls"):
    ...
    if method == "gpt_cls":
        ...
        for ref in reference:
            if ref == ans:
                return True
            e1 = check_entailment(item['question'], ans, ref)
            e2 = check_entailment(item['question'], ref, ans)
            if e1 == e2 == 1:
                return True
        return False
    else:
...
class Collab:
            ...
            for key, agent_group in initial_agents.items():
                for agent in agent_group:
                    ...
                    if "Abstain" in res:
                        ...
                        correctness = eval_ans(self.dataset_name, item, extract_ans(res), item["ground_truth"], method = "gpt_cls")
    ...
    def construct_stances(self, votes,
        ...
        if filter_abstain:
            kept = [
                (vote, rationale)
                for vote, rationale in zip(votes, conf_rationales)
                if vote[1] is not None and "abstain" not in str(vote[1]).lower()
            ]
            votes = [x[0] for x in kept]
            conf_rationales = [x[1] for x in kept]
        for i, (id, ans, verb_conf, *_) in enumerate(votes):
            ...
            if i == 0:
                ...
                for unique_class in classes.keys():
                    if eval_ans(self.dataset_name, record, ans, unique_class, method="nli_cls"):
    ...
    def deliberate_with_feedback(self, record, agents: List[BaseAgent],
        ...
        final_votes_raw = self.revote(record, agents_observations_mapping)
        rationales = [vote[-1] for vote in final_votes_raw]
        final_set = self.construct_stances(
