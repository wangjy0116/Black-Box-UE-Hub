...
{{
  ...
  "final_answer": "Your final answer"
    ...
    def compute_cota(self, question, steps_a, steps_b):
    ...
    def calculate(self):
        ...
        if "cots" not in dataset[0].get("sample_results", []):
        ...
        for item in tqdm(dataset):
            ...
            for i in range(self.sample_num):
                ...
                if sample_cots is not None and sample_cots != []:
                    sum_score += self.compute_cota(item["question"], greedy_cots, sample_cots)
                    cnt+=1
            if cnt <= valid_num:
                cota_scores.append(None)
            else:
