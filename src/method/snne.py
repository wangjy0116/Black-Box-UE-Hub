...
from src.utils import compute_sample_semantic, build_sample_path, sample_generate, extract_final_answer
...
{{
  ...
  "final_answer": "Your final answer"
    ...
    def uf_labels_from_classmat(class_mat: np.ndarray) -> np.ndarray:
        """
        class_mat: [K,K], where class_mat[i,j] == 1 means semantic-equivalent.
        Return labels: [K] in {0..num_clusters-1}
        """
        K = class_mat.shape[0]
        parent = list(range(K))
    ...
    def compute_and_cache_semantics(
        ...
        for item in tqdm(dataset):
            ...
            E, C, N, E_logits, C_logits, N_logits, class_mat, _ = compute_semantic_matrices(
                question=question,
    ...
    def cal_auroc(self, y_true: List[int], y_score: List[float]) -> float:
        ...
        if len(np.unique(y_true)) < 2:
            return error("Only one class present in y_true. ROC AUC score is not defined in that case.")
